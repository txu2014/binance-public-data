...
def get_destination_dir(file_url, folder=None):
    ...
    #store_directory = '/mnt/g/i_download/binance/'
    if folder:
...
def delete_file(base_path, file_name, date_range=None, folder=None):
...
def download_file(base_path, file_name, date_range=None, folder=None):
    ...
    logging.info([download_url, save_path])
    urlretrieve(download_url, save_path)

    # except urllib.error.HTTPError as ex:
    #     #logging.error("\nFile not found: {}".format(download_url))
    #     logging.error([download_url, ex])
...
def get_parser(parser_type):
    ...
    parser.add_argument(
        '-y', dest='year', default=YEARS, nargs='+', choices=YEARS,
    ...
    parser.add_argument(
        '-t', dest='type', default='um', choices=TRADING_TYPE,
